...
| ADR-004 | CI 使用 `--bare` | **已被 ADR-016 取代** | GitHub Actions 需要可复现，不加载本地 hooks/MCP/CLAUDE | 加载项目 hooks，但 CI 不稳定 |
...
| ADR-016 | 单轮、受限、可验证的 Step Plan 自举链路 | 接受（2026-07-13） | 取代 ADR-004，并更新 ADR-005/013 的当前实施边界：owner 自然语言经确认卡后，由前端内部发送精确 `/create`；每次远程运行只执行一次 StepPlan Plan 和一次受限 Claude Execute。Claude 客户端最终只到 `https://api.stepfun.com/step_plan/v1/messages`，聊天只到 `https://api.stepfun.com/step_plan/v1/chat/completions`，普通 `/v1` 禁止。Execute 无 shell、Web、MCP、子 Agent 或项目技能；候选仅以经过密钥扫描的 patch/hash/manifest 传递并保留 1 天，由 fresh independent verifier 从不可变 base 验证，成功后创建 draft PR，人工 merge 才能发布 | 多次模型重试、多轮 workflow 自触发、上传 prompt/模型响应/原始日志、verifier 与生成进程共享凭据 |

ADR-004、ADR-005 和 ADR-013 的原始行保留为历史决策记录；发生冲突时，当前 CI 实施以 ADR-016 为准。
