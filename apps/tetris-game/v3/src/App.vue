<!-- Tetris Game v3 - Enhanced with Ghost Piece -->
...
const COLORS: Record<number, string> = {
  ...
  6: '#2979ff',  // J - Blue
  7: '#ff9100',  // L - Orange
  8: 'rgba(255, 255, 255, 0.15)', // Ghost piece
}
...
function getGhostY(piece: Piece, board: CellValue[][]): number {
  let ghostY = piece.y
  while (true) {
...
const displayBoard = computed<CellValue[][]>(() => {
  const display: CellValue[][] = state.board.map((row) => [...row])

  // Render ghost piece first (underneath)
  if (state.currentPiece) {
    const ghostY = getGhostY(state.currentPiece, state.board)
      ...
      for (let r = 0; r < p.shape.length; r++) {
        for (let c = 0; c < p.shape[r].length; c++) {
          if (p.shape[r][c]) {
            const ny = ghostY + r
            const nx = p.x + c
            if (ny >= 0 && ny < ROWS && nx >= 0 && nx < COLS && display[ny][nx] === 0) {
              display[ny][nx] = 8
            }
          }
        }
...
function handleKeyDown(e: KeyboardEvent): void {
  ...
  if (!state.isRunning && isBoardEmpty()) {
...
<template>
  <div class="game-container" @touchstart="handleTouchStart" @touchend="handleTouchEnd">
    ...
    <header class="game-header">
      ...
      <span class="game-subtitle">俄罗斯方块 v3</span>
    </header>
    ...
    <div class="game-layout">
            ...
            <template v-if="holdShape">
              ...
              >
                <div
                  ...
                  class="hold-cell"
                  :class="{
                    filled: holdShape && holdShape[r]?.[c],
                    dimmed: !state.canHold && state.isRunning,
                  }"
                  :style="
                    holdShape && holdShape[r]?.[c]
      ...
      <main class="board-wrapper">
        <div class="board-container">
          ...
          <div
            v-if="clearMessage"
            class="clear-message"
            :class="{ tetris: clearMessage.includes('TETRIS') }"
          >
            {{ clearMessage }}
          </div>
          ...
          <!-- Idle Overlay -->
          <div
            v-if="!state.isRunning && !state.isGameOver && isBoardEmpty()"
      ...
      <aside class="side-panel right-panel">
            ...
            <template v-if="nextShape">
              ...
              >
                <div
                  ...
                  class="next-cell"
                  :class="{
                    filled: nextShape && nextShape[r]?.[c],
                  }"
                  :style="
                    nextShape && nextShape[r]?.[c]
        ...
        <div class="panel-box actions-box">
          ...
          <p class="bootstrap-hint">这是由 AI 构建的应用。登录后可以在此基础上继续迭代。</p>
        </div>
      </aside>
    </div>
  </div>
