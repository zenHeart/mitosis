...
│                    → GitHub Pages 自动部署                       │
│                                                                 │
│  Pages URL: blog.zenheart.site/mitosis/  ← 直接由此仓库服务    │
│                                                                 │
│  DNS: blog.zenheart.site CNAME → zenHeart.github.io (已配置)   │
│  Pages Source: zenHeart/mitosis /master 或 /gh-pages 分支      │
└─────────────────────────────────────────────────────────────────┘

关键变更：不跨仓库推送。zenHeart/mitosis 自身启用 GitHub Pages，
通过 DNS CNAME 映射到 blog.zenheart.site/mitosis/。
```
...
- [ ] 构建产物在 `zenHeart/mitosis` 仓库的 GitHub Pages 上可访问（`blog.zenheart.site/mitosis/apps/{name}/v{n}/`）
- [ ] Issue 自动关闭并附带完成评论
...
| # | 问题 | 建议值 |
|---|------|--------|
| 4 | `blog.zenheart.site/mitosis/` 的 DNS CNAME 是否已指向 `zenHeart.github.io`？ | 已配置 |
