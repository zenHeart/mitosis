...
### C8: 功能约束（用户明确要求移除的功能必须不存在）
- [ ] **无 ghost piece** — 无虚线预览元素 (`.ghost-piece`, `[class*="ghost"]`)
- [ ] **无粒子效果** — 无粒子容器或粒子 DOM (`.particles-container`, `.particle`)
- [ ] **无 glow/bloom** — 无过量 box-shadow（>5 个 inline box-shadow = 性能问题）
- [ ] **无已删除应用** — Gallery 和代码中无 snake-game 等已删除应用引用
- [ ] **FPS >= 55** — 活跃游戏期间平均帧率 >= 55fps（目标 60fps）

> **铁律：** 用户在对话中明确说"去掉 X" → 必须在 `goal.md` Feature Constraints 中记录 → Verifier 自动检查 X 不存在。
> 未记录的约束不会被验证，Executer 可能反复添加再移除。
