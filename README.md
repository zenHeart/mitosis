...
```yaml
# .github/workflows/mitosis.yml — 每个用户仓库自动生成
name: Mitosis Build
on:
  issues:
  ...
  build:
    ...
    steps:
      ...
      - name: Run Agent Loop
        ...
        run: |
          claude --api-key $STEP_TOKEN \
            --working-dir . \
            --prompt "Build the app described in issue #${{ github.event.issue.number }}"
      - name: Determine next version
        id: version
        run: |
          # 计算下一个版本号 v0, v1, v2...
          EXISTING=$(ls apps/ 2>/dev/null | grep -c '^v')
          if [ "$EXISTING" -eq 0 ]; then
            echo "next_version=v0" >> $GITHUB_OUTPUT
          else
            NEXT=$(($(ls apps/ | grep '^v' | sed 's/v//' | sort -n | tail -1) + 1))
      ...
      - run: |
          ...
          # 提交到版本目录，不覆盖已有版本
          git add apps/
          git commit -m "feat: build v${{ steps.version.outputs.next_version }} from issue #${{ github.event.issue.number }}"
          # 更新默认版本软链接
          rm -rf apps/latest
          ln -s v${{ steps.version.outputs.next_version }} apps/latest
          git add apps/latest
          git commit -m "chore: update latest to v${{ steps.version.outputs.next_version }}"
          git push
```

> **版本化策略：** 每次构建生成新版本目录 `v0`, `v1`, `v2`...，不覆盖已有版本。`apps/{name}/latest/` 软链接始终指向最新版本。Pages URL 中 `/apps/{name}/` 映射到 `latest`，`/apps/{name}/v1/` 等可访问历史版本。
...
sequenceDiagram
    ...
    GitHubActions->>ClaudeCode: 检出代码 + 安装依赖

    Note over ClaudeCode: 读取 GitHub Secrets<br/>获取 STEP_TOKEN

    ClaudeCode->>ClaudeCode: 分析需求 → 生成项目计划
    ClaudeCode->>GitHub: 创建 apps/{name}/v{n}/index.html
    ClaudeCode->>GitHub: 创建 apps/{name}/v{n}/style.css
    ClaudeCode->>GitHub: 创建 apps/{name}/v{n}/main.ts
    ClaudeCode->>GitHub: 创建 apps/{name}/v{n}/vite.config.ts
    ClaudeCode->>GitHub: 创建 apps/{name}/v{n}/tsconfig.json
    ClaudeCode->>GitHub: 提交新版本 + 更新 latest 软链接

    GitHub->>GitHubActions: Push 触发 Pages 构建
    GitHubActions->>GitHub: Pages 构建完成

    Mitosis前端->>GitHub: 轮询 Issue 状态
    GitHub->>Mitosis前端: Issue 关闭 + 构建成功
    Mitosis前端->>用户: 构建完成！应用已部署（v{n}）
```
