    ...
    steps:
      ...
      - name: Run Agent Loop
        ...
        run: |
          claude --api-key $STEP_TOKEN \
            ...
            --max-turns 50 \
            --prompt "构建 apps/\$APP_NAME/v0/ 下的应用..."
      - name: Commit and push
        run: |
          ...
          git commit -m "feat: build \${{ steps.parse.outputs.app_name }} v0 (issue #\${{ github.event.issue.number }})"
          git push origin master
      - name: Close issue
        run: |
          gh issue close ${{ github.event.issue.number }} \
            --repo ${{ github.repository }} \
            --comment "✅ 构建完成！访问: https://mitosis.zenheart.site/apps/\${{ steps.parse.outputs.app_name }}/"
```
