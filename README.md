...
| 理解 CI Loop | [docs/agent-loop.md](docs/agent-loop.md) | 单次 Plan→Execute、独立验证、安全门控 |
...
4. owner 在 Workspace 输入自然语言；平台完成意图分流并展示确认卡，只有 owner 确认后才创建 GitHub Issue，并由前端在后台发送精确的 `/create` 评论触发 CI。普通用户不需要输入命令。
5. GitHub Actions 在受限沙盒中执行一次 StepPlan Plan 和一次 Claude Code Execute；Execute 只能使用文件读写工具，不能使用 shell、Web、MCP、子 Agent 或项目技能，也不会失败后自动重试或自触发下一轮。
6. 生成 job 只输出经过密钥扫描的 `candidate.patch`、SHA-256 和 manifest，保留 1 天；fresh verifier job 从不可变 base 应用并验证同一候选，且拿不到 `STEP_TOKEN` 或写权限。
7. 独立验证通过后才创建 draft PR；人工审查合入 `master` 后由 Pages 发布。

> **StepFun 强约束：** 所有模型请求只能使用 Step Plan 路径。Claude Code 的 `ANTHROPIC_BASE_URL` 必须使客户端最终请求命中 `https://api.stepfun.com/step_plan/v1/messages`；浏览器聊天只使用 `https://api.stepfun.com/step_plan/v1/chat/completions`。StepFun 根域下的普通 `/v1/...` 路径禁止使用。

```text
匿名使用 → 登录鉴权 → owner 设置 / 其他用户复制仓库
→ 自然语言 → 确认卡 → GitHub Issue → 内部精确 /create → owner gate
→ 一次 Plan → 一次受限 Execute → 密钥扫描后的 patch/hash/manifest
...
- 确认卡确认后由前端在后台触发 Mock 构建，普通用户无需输入 `/create`
- 数据存储在 `mitosis_mock_sessions` 和 `mitosis_mock_sessions_messages`，可通过 `clearMockData()` 重置

> 注意：Mock 模式下 StepFun AI 对话仍需要有效的 `VITE_STEP_TOKEN`。完全离线的流程验证应使用仓库测试，不把手工命令当作 MVP 用户路径。
